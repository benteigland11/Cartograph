"""
Tests for BM25 search, filtering, and result formatting.
"""
import pytest
...
def test_search_finds_widget_by_name(carto):
    ...
    ids = [w["id"] for w in result["library"]]
...
def test_search_type_filter_widget(carto):
    result = carto.search("api", type_filter="widget")
    types = [w.get("type", "widget") for w in result["library"]]
    # All results should be widgets, not blueprints
    assert all(t == "widget" for t in types)


def test_search_type_filter_blueprint(carto):
    result = carto.search("api", type_filter="blueprint")
    # web-api-stack blueprint should appear
    ids = [w["id"] for w in result["library"]]
    assert "web-api-stack" in ids


def test_search_result_fields(carto):
    result = carto.search("http")
    for item in result["library"]:
        assert "id" in item
        assert "name" in item
...
def test_search_sorted_by_relevance(carto):
    ...
    scores = [w["relevance_score"] for w in result["library"]]
    assert scores == sorted(scores, reverse=True)


def test_search_empty_library():
    """Search on a Cartographer with no widgets returns empty result."""
    from cartographer import Cartographer
    import tempfile, os
    with tempfile.TemporaryDirectory() as tmp:
        # Create a minimal valid but empty library dir
        lib = os.path.join(tmp, "Widget_Library")
        os.makedirs(lib)
        c = Cartographer(library_path=lib)
        result = c.search("anything")
    assert result == {"installed": [], "library": []}


def test_search_top_k_respected(carto):
    result = carto.search("backend", top_k=2)
    total = len(result["library"]) + len(result["installed"])
    assert total <= 2
