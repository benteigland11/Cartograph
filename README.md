...
- **Flutter**: Flutter SDK 3.x (bundles its own Dart - the whole toolchain in one install), `flutter test` + flutter_test harness with lcov line coverage (80%), `flutter analyze` gate, native comment/string-aware Dart scanner (handles Dart's nested block comments and raw strings). Widgets own their pubspec.yaml (two marked dependency blocks are generated); sources live in src/ and the engine generates lib/ from it at validation time because Flutter's coverage collector only reports lib/. Examples run headless through the flutter_test harness (Flutter UI can't execute on the bare Dart VM). Pure Dart logic widgets validate through the same path. `print`/`debugPrint`, `exit()`, and blocking `sleep()` are blocked in src/. Blueprints compose via pub path deps.
...
Run `cartograph doctor` to check that all language engine dependencies (pytest, coverage, node, npx, vitest, nim, nimble, openscad, iverilog, ng, php, composer, terraform, go, cargo, cargo-llvm-cov, ngspice, godot, java, gradle, lean, lake, dotnet, flutter, dart) are installed correctly.
