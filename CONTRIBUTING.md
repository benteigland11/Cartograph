...
**Lean-specific notes:** Lean 4 widgets are lake packages (`lakefile.toml` with a single `[[lean_lib]]`, `srcDir = "src"`, lib name = the widget slug with underscores). The build is the validator: `lake build` kernel-checks every theorem, so a successful validation means the widget's stated properties are proven. Two compiler leniencies are closed explicitly - `sorry`/`admit` (compiles with only a warning) is hard-blocked everywhere by the native scanner and by parsing build output, and custom `axiom` declarations are hard-blocked in src/. Tests define `main` and follow the ASSERT_PASS contract (like GDScript); proof-level assertions in test files are kernel-checked during elaboration on the same run. No coverage floor - no tool exists, and the kernel check is stronger for proof code. v1 is dependency-free (Lean core library only): `install_deps` rejects declared dependencies with a clear error. Mathlib is a deliberate future decision (multi-gigabyte, toolchain-pinned, needs a pre-installed-cache policy like the ML frameworks). Widgets do not carry a `lean-toolchain` pin; the machine's default elan toolchain validates and its version is stamped at checkin.
